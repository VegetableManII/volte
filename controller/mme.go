...
func (this *MmeEntity) AttachRequestF(ctx context.Context, p *common.Package, up, down chan *common.Package) error {
	...
	common.PackUpEpcMsg(p.CommonMsg, common.EPCPROTOCAL, common.AuthenticationInformatRequest, req, host, up) // 上行
	return nil
}

// HSS 响应Authentication Informat Response，拿到用户签名XRES、HSS服务器的Auth信息、随机数nonce和加密方法Kasme
func (this *MmeEntity) AuthenticationInformatResponseF(ctx context.Context, p *common.Package, up, down chan *common.Package) error {
	defer common.Recover(ctx)
	...
	common.PackUpEpcMsg(p.CommonMsg, common.EPCPROTOCAL, common.AuthenticationInformatRequest, args, host, down) // 下行
	return nil
}

// UE终端 响应AuthenticationResponse，比较用户RES是否与XRES一致
func (this *MmeEntity) AuthenticationResponseF(ctx context.Context, p *common.Package, up, down chan *common.Package) error {
	defer common.Recover(ctx)
	...
	if res != xres {
		// 鉴权失败，重新发起鉴权请求
		host := this.Points["eNodeB"]
		common.PackUpEpcMsg(p.CommonMsg, common.EPCPROTOCAL, common.AuthenticationInformatRequest, nil, host, down)
		return errors.New("ErrAuthenFailed")
	}
	// 向上行HSS发送位置更新请求
	host := this.Points["HSS"]
	common.PackUpEpcMsg(p.CommonMsg, common.EPCPROTOCAL, common.UpdateLocationRequest, nil, host, up) // 上行
	return nil
}

// 接收HSS的 Update Location ACK 响应得到用户的APN，再请求PGW完成承载的建立（该部分暂不实现，得到ACK后直接向用户发送Attach Accept）
func (this *MmeEntity) UpdateLocationACKF(ctx context.Context, p *common.Package, up, down chan *common.Package) error {
	defer common.Recover(ctx)
	...
	*/
	...
	ueip := args["IP"]
	// 请求PGW建立IMS信令承载
	common.PackUpEpcMsg(p.CommonMsg, common.EPCPROTOCAL, common.CreateSessionRequest, args, pgwaddr, up)
	// 响应UE终端附着允许的响应
	host := this.Points["eNodeB"]
	t2 := map[string]string{
		"IP": ueip,
	}
	common.PackUpEpcMsg(p.CommonMsg, common.EPCPROTOCAL, common.AttachAccept, t2, host, up)
	return nil
}
